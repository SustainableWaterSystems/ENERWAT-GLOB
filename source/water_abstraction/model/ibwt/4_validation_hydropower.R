...
#### modelled hydropower
reservoirs.ibwt <- read.csv(paste0(inputDirDischarge, '2_infrastructure/1_information_hydropower.csv'))

hydropower.year <- read.csv(paste0(inputDirEnergy, '1_energy_production_yearly.csv')) %>% 
  mutate(datetime = as.Date(datetime))

hydropower.month <- read.csv(paste0(inputDirEnergy, '1_energy_production_monthly.csv'))%>% 
  mutate(datetime = as.Date(datetime)) %>% 
  mutate(
