...
inputDirDischarge <- paste0(inputDir, '1_discharge_timeseries/0_timeseries_pcrglobwb/')

outputDirData <- paste0(inputDir, '1_discharge_timeseries/1_bias_corrected/')

#### load ####
section.information <- read.csv(
  paste0(outputDirData, '0_information_sections.csv'))
...
write.csv(bias.corrector.unique,
          paste0(outputDirData, '1_0_bias_correction_sections.csv'), row.names = F)
write.csv(data.segments.unique,
          paste0(outputDirData, '1_1_bias_correction_segments.csv'), row.names = F)
